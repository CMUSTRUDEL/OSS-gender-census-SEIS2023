...
def commit_census(lang):
    ...
    for pid in projs:
        #log.write(str(pid)+"\n")
        try:
            commits = pickle.load(open(commit_path+lang+"/"+pid, "rb")) # {win:{uid:[#commits, gender]}}
            for win in commits:
                uid_dict = commits[win] # {uid:[#commits, gender]}
                if win not in commit_dict:
                    commit_dict.update({win:[0,0,0]})

                for uid in uid_dict:                     
                    commit_dict[win][2] += uid_dict[uid][0]
                    if uid_dict[uid][1] == 1:
                        commit_dict[win][0] += uid_dict[uid][0]
                    elif uid_dict[uid][1] == -1:
                        commit_dict[win][1] += uid_dict[uid][0]
        except:
            continue
