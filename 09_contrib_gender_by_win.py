...
def contributor_census(lang):
        ...
        if i in all_dict:
            for uid in all_dict[i]:
                ...
                if gender is None:
                    ...
                    if gender.probability >= 0.8:
                        female_all += 1
                        all_dict[i].update({uid:1})
                    elif gender.probability >= 0.63:
                        female_likely_all += 1
        ...
        if i in core_dict:
            for uid in core_dict[i]:
                ...
                if gender is None:
                    ...
                    unknown_core += 1
        dat.append([i, male_all, male_likely_all, female_all, female_likely_all, unknown_all, all_contrib, male_core, female_core, unknown_core, core_contrib]) 
    ...
    dat = pd.DataFrame(dat, 
            columns = ["win", "male_all", "male_likely_all", "female_all", "female_likely_all", "unknown_all", "all_all", "male_core", "female_core", "unknown_core", "all_core"])
